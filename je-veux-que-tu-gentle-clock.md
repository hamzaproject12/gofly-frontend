...
- **Dernier commit** : `66d931c` (merge) / `f0d94a5` — **page `/programmes/modifier/[id]` alignée sur `/programmes/nouveau`** : onglets Madina/Makkah/Autre + compteur de lits, NB jours dans l'en-tête d'onglet, onglet Autre (charge `program.hotelsAutre`, nb nuits éditable, +/- chambres bornés par places occupées, prix) ; PUT envoie `hotelsAutre`. Round-trip vérifié au runtime (PUT prog 36 : nbJours + ajout chambre Autre OK, Madina/Makkah intacts). Avant : `67c1d15` (Phase 4 reporting), `79a1334`, `88d5c36`, `4d69dfa`, merge `735fdbb` (Phases 1-3, prod).
